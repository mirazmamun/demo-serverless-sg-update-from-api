...
class SG {
    ...
    /**
     * Remove security group if exists
     * 
     * @param {String} sgName 
     * @return {Promise}
     */
    removeSG(sgName = null) {
        const self = this;
        return new Promise((resolve, reject) => {
            waterfall(
                [
                    ...
                    (sg, vpcID, cb) => {
                        ...
                        if (sg) {
                            ...
                            self.ec2.deleteSecurityGroup(params, function (err, data) {
                                err ? cb(err) : cb(null, data);
                            });
                        } else {
                            cb(null, `Security group does not exist, so no action taken`);
                        }
                    }
                ],
                (finalErr, finalRes) => {
